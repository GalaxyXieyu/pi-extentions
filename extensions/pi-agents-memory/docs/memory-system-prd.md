...
本 PRD 定义 Pi 长期记忆系统（pi-agents-memory）应该"做什么、为什么做、做到什么程度"，用于对齐实现、评审、迭代。
...
- 工具闭环：`memory_review`/`memory_review`（list + 裁决）
...
- 命令 `memory-consolidate`/`memory-consolidate`：扫描账本同 kind+scope 高相似（dice≥0.6）active 记录，冲突/重复对主动提升 pending_review
- 非破坏性：只标记，不删除

### FR-9 观测与审计 ✅

- capture/review/consolidation/error 全部记入 memoryStats（jsonl）
- 审计命令 `memory-audit`（脱敏后）
...
| **L3 全文/事实点** | 记忆全文文件、viking:// 资源                           | 大     | 只在需要时读（memory_read/archive_expand），不进每轮注入                             |
...
| L3  | 显式 memory_read/browse/archive_expand                | 无 URI 体系（云端无全文族）                           | —                                         |
| L4  | 未实现                                                 | 未实现                                        | —                                         |
...
3. **L3 按需泛化**：把 memory_read 工具与 L2 命中绑定（分数超阈值且 detail=abstract 时给出"读全文"hint）
