...
export function extractMessages(branch: unknown[], syncedEntryCount: number, config: VikingMemoryConfig): CaptureResult {
  ...
  for (const entry of entries.slice(start)) {
    ...
    if (!safeText || isOwnSlashCommand(safeText) || safeText.startsWith("[Viking Memory]") || safeText.startsWith("[Agents Memory]")) continue;
...
/**
 * The plugin's own slash-command echoes must never become memories. Matching
 * stops at the command token, so prose that merely starts with "/memory" is
 * still captured.
 */
const OWN_COMMAND_PREFIXES = ["/memory", "/viking-memory", "/viking"];

export function isOwnSlashCommand(text: string): boolean {
  if (!text.startsWith("/")) return false;
  const boundary = text.search(/\s/);
  const head = (boundary === -1 ? text : text.slice(0, boundary)).toLowerCase();
  return OWN_COMMAND_PREFIXES.some((prefix) => head === prefix || head.startsWith(`${prefix}-`));
}
