...
import { authorize, curateWithLlm, filterRecall, gateCapture, persistLifecycleRecord, persistLifecycleTransition, backfillLifecycleRemoteId, memoryHistoryById, memoryStats } from "../../core/runtime.js";
...
export class VikingMemoryProvider implements MemoryProvider {
  ...
  async capture(sessionId: string, messages: import("../../core/provider.js").MemoryMessage[], context?: MemoryRequestContext): Promise<CaptureResult> {
    ...
    for (const message of messages) {
      ...
      sessionMessages.push({
        ...
        // Rule-miss long tail. By default nothing happens here: the message is
        // pushed into the cloud session and the nightly sweep re-reads the
        // transcript later. Only the explicit opt-in (PI_MEMORY_LLM_INLINE=1)
        // keeps the in-session batch funnel alive.
        if (inlineLlmEnabled()) this.curationQueue.enqueue({ role: message.role, content: message.content });
        else memoryStats.record({ type: "extraction", backend: this.id, operation: "rule-miss", requestId: context?.requestId });
        sessionMessages.push({
          ...message,
          metadata: context ? {
            ...(message.metadata || {}),
   ...
   */
  async curateBatch(batch: import("../../core/provider.js").MemoryMessage[], sessionId: string, context: MemoryRequestContext): Promise<{ handled: boolean; count: number; rejected: number; decisions: Array<{ decision: string; reason: string; kind?: string }>; error?: string }> {
    const decisions: Array<{ decision: string; reason: string; kind?: string }> = [];
