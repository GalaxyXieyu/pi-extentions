...
import { createVikingNightlyProvider } from "./nightly.js";
...
export default async function (pi: ExtensionAPI) {
  ...
  // --- nightly sweep (in-process: pi loads these sources fine; plain node
  // cannot type-strip TypeScript from inside node_modules, so a standalone
  // node script would only work in a repo checkout, never from an npm install)
  const runNightly = async (ctx: any, args: string) => runNightlyInProcess({
    ...parseNightlyArgs(args),
    base: { userId: config.userId, agentId: config.assistantId },
    complete: makeHostComplete(() => ctx),
    providerFor: createVikingNightlyProvider(config),
    log,
  });
  ...
  registerMemoryCommand(pi, "memory-capabilities", {
  ...
  registerMemoryCommand(pi, "memory-stats", {
  ...
  registerMemoryCommand(pi, "memory-consolidate", {
  ...
  registerMemoryCommand(pi, "memory-nightly", {
    ...
    handler: async (args: string, ctx: any) => {
      ctx.ui.notify("Agents Memory: 正在离线摸查近期会话（后台跑，不影响你继续聊）…");
  ...
  registerMemoryCommand(pi, "memory-audit", {
  ...
  registerMemoryCommand(pi, "memory-workspace", {
  ...
  registerMemoryCommand(pi, "memory", {
    ...
    handler: async (args: string, ctx: any) => {
      ...
      ctx.ui.notify(`Agents Memory v${EXTENSION_VERSION} (backend: Viking Memory): ${connected ? "connected" : "disconnected"} | collection=${config.collectionName} | workspace=${workspace.id} (${workspace.source}) | conversation=${conversationId || "none"} | capture=rules-only ${inlineLlmEnabled() ? "+inline-llm" : ""}, llm=nightly-sweep`, connected ? "info" : "warning");
    },
  });
}
...
function setStatus(ctx: any, connected: boolean, config: VikingMemoryConfig, synced: number): void {
  try {
    ctx?.ui?.setStatus?.(`Memory ${connected ? "✓" : "✗"} · ${config.collectionName} · ↕${synced}`);
  } catch {
