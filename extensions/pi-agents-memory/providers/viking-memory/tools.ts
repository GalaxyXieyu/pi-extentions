...
export function registerTools(
  ...
  pi.registerTool({
    ...
    promptSnippet: "Search the connected memory backend for past preferences, events, and decisions",
  ...
  pi.registerTool({
    ...
    promptSnippet: "Remember a durable fact in the connected memory backend",
    parameters: Type.Object({
    ...
    async execute(_id: string, params: any) {
      ...
      if (!result.accepted) return { content: [{ type: "text", text: result.error || "memory request failed." }], isError: true };
      return { content: [{ type: "text", text: "Remembered in the connected memory backend." }], details: { success: true, count: result.count, backend: result.backend } };
    },
    ...(card ? { renderResult: card } : {}),
  });
  ...
  pi.registerTool({
    ...
    description: "Upsert a global user preference/profile in the connected memory backend.",
    promptSnippet: "Update the user profile in the connected memory backend",
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    promptSnippet: "Review pending memory contradictions",
    parameters: Type.Object({
