...
```
~/.pi/agent/extensions/pi-agents-memory/
...
Inline in `index.ts`. Loaded from `~/.pi/agent/extensions/pi-agents-memory/config.json`.

```typescript
interface OVConfig {
  enabled: boolean;              // Master switch (default: true)
  endpoint: string;              // OV server URL (default: "http://127.0.0.1:1933")
  apiKey: string;                // API key resolved from environment/secure ovcli.conf only
...
Config resolution: behavior knobs come from the extension `config.json`; credentials come from environment variables or owner-only `~/.openviking/ovcli.conf`. Insecure credential files are ignored.
...
Remote deletion is intentionally outside this plugin; corrections require a separate, explicit administrative workflow.
...
class OVClient {
  ...
  // Remote deletion is intentionally unavailable in the Pi adapter.
...
- (8 more — use memory_search to find specific memories)
...
- (1 more — use memory_browse to explore)

### viking://user/sessions/{session_id}/history/ (2 archives)
- Archive 2026-05-25: 15-turn session about pi extension design
- (1 more — use memory_archive_expand for detail)

Tools: memory_search | memory_read | memory_browse | memory_remember | memory_add_resource | memory_archive_expand
```
...
The model sees the index and knows "OV knows about X, Y, Z." When a task touches those topics, it uses `memory_search` for depth or relies on automatic `<relevant-memories>` injection.
...
9. **Token-budgeted formatting with graceful degradation** (from Claude Code plugin):
   ...
   - Items beyond the budget are **degraded to URI + score hints** rather than dropped — the model can call `memory_read` to expand them
...
- [skill 0.73] Use memory_read to expand: viking://user/skills/deployment-checklist.md
</relevant-memories>
```

The third line shows a degraded hint — the item was beyond the content budget but still relevant. The model can expand it with `memory_read` if needed.
...
```typescript
{
  ...
  promptGuidelines: [
    "Use memory_search when you need information from previous sessions that may not be in MEMORY.md.",
    "Use memory_search before making decisions that might conflict with established patterns or past decisions.",
  ],
...
```typescript
{
  ...
  promptGuidelines: [
    "Use memory_remember for facts that should survive across sessions but don't belong in MEMORY.md.",
...
Memory deletion is intentionally not exposed by the Pi plugin; corrections must go through an explicit server-side administration workflow outside this extension.
...
4. [Turns execute — LLM may call memory_search, etc.]

5. agent_end fires
   a. clear the provider recall block and pending prompt
```
...
Default: `~/.pi/agent/extensions/pi-agents-memory/config.json`
