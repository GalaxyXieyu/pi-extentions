...
test("pi URI guard blocks builtin file tools on viking URIs", () => {
  ...
  assert.match(decision?.reason ?? "", /Use memory_read instead/)
})
...
test("pi URI guard blocks bash commands containing viking URI", () => {
  ...
  assert.equal(decision?.block, true)
  assert.match(decision?.reason ?? "", /Use memory_read or memory_search instead/)
})
...
test("pi URI guard allows normal local paths and OpenViking native tools", () => {
  ...
  assert.equal(guardVikingUriToolCall({ toolName: "memory_read", input: { uri: "viking://resources/file.md" } }), null)
})
