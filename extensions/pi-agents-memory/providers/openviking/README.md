...
Set the unified backend selector before starting Pi:

```bash
export PI_MEMORY_BACKEND=openviking
```

Install the checkout directly from this aggregation repository:

```bash
pi install ./extensions/pi-agents-memory
```

The extension is registered with Pi and loads on the next invocation.

### 3. Configure (optional)

`PI_MEMORY_BACKEND=openviking` is required when the Viking Memory plugin is also installed. This prevents duplicate recall and writes. The shared provider contract is used for health, search and capture shapes; OpenViking-specific session commit, `viking://` resources and context takeover continue through the native OpenViking path.
...
```bash
node ./extensions/pi-agents-memory/providers/openviking/scripts/setup.mjs
```

`~/.pi/agent/extensions/pi-agents-memory/providers/openviking/config.json` is for behavior knobs only:
...
The extension shows an `[OpenViking]` status line on startup. Tools (`memory_search`, `memory_remember`, etc.) are registered automatically. Memories persist across sessions — no additional setup.
...
| Tool                     | Description                                                |
|--------------------------|------------------------------------------------------------|
| `memory_search`          | Semantic search across memories, resources, and skills     |
| `memory_read`            | Read a `viking://` URI at abstract / overview / full level |
| `memory_browse`          | List directory contents or stat a `viking://` URI          |
| `memory_remember`        | Store a fact or preference into long-term memory           |
| `memory_add_resource`    | Ingest a URL into OpenViking for indexed retrieval         |
| `memory_archive_expand`  | Expand an archived session back into raw conversation      |

The canonical `/memory` command (type `/memory` in pi's chat) displays connection status, session info, and accepts `commit` for manual synchronous commit.
...
├── tools.ts             # 7 registered LLM tools + /memory command
...
| Takeover never advances                  | Pending addMessage replay, commit, or overview polling failed | Set `OV_DEBUG_LOG=/tmp/ov-pi.log` and retry `/memory commit` |
