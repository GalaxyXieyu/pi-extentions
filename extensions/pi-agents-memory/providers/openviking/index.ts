...
export default async function (pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    parts.push("OpenViking tools: memory_search, memory_read, memory_browse, memory_remember, memory_add_resource, memory_archive_expand.");
  ...
  registerMemoryCommand(pi, "memory", {
    ...
    handler: async (args, ctx) => {
      ...
      const takeoverInfo = config.takeoverEnabled
        ...
        : "";
      ctx.ui.notify(
        `OpenViking: ${connected ? "connected" : "disconnected"} | provider=${provider.id} | session: ${sid.slice(0, 12)}...${takeoverInfo} | capture=rules-only ${inlineLlmEnabled() ? "+inline-llm" : ""}, llm=nightly-sweep`,
        "info",
      );
    },
  });
