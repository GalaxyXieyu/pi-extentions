...
export async function curateWithLlm(
...
): Promise<{ handled: boolean; decisions: LlmCaptureDecision[]; error?: string }> {
  if (!llmExtractionEnabled()) return { handled: false, decisions: [], error: "PI_MEMORY_LLM_ENABLED=0" };
  // Needs at least one completion source: pilot hook or an endpoint.
  if (!complete && !process.env.PI_MEMORY_LLM_URL) return { handled: false, decisions: [], error: "no completion source: set PI_MEMORY_LLM_URL or run inside pi" };
  const query = clipQuery(messages);
  if (!query) return { handled: false, decisions: [], error: "empty batch" };
  ...
  if (!result.ok || result.fallbackToRules) {
    ...
    return { handled: false, decisions: [], error: result.error || "llm-unavailable" };
  }
 ...
 *   for the `memory_review` tool or a later session.
...
export async function handleReviewPrompts(
  ...
  if (mode === "confirm" && typeof ui?.select === "function") {
    ...
    for (const conflict of conflicts) {
      ...
      try {
        choice = await ui.select(`记忆冲突（Agents Memory）:新记忆与已有记忆内容互相矛盾,请选择如何处理\n\n新记忆:「${newText}」\n已有记忆:「${oldText}」`, [
          "accept-new (采纳新记忆)",
          "keep-old (保留旧记忆)",
          "merge (合并两者)",
          "defer (暂不处理)",
  ...
  if (typeof ui?.notify === "function") {
    ui.notify(`Agents Memory:有 ${conflicts.length} 条新记忆与已有记忆冲突,待确认。可直接对我说"处理记忆冲突",或运行 memory_review 工具。`);
  }
...
export async function runConsolidationPass(identity: MemoryIdentity, ui?: any): Promise<{ findings: ConsolidationFinding[]; promoted: number }> {
  ...
  if (ui?.notify && promoted > 0) ui.notify(`Agents Memory: consolidation flagged ${promoted} similar/contradicting memor(ies) for review.`);
