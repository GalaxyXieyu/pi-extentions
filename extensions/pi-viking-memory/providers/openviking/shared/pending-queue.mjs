...
import { sanitizeSensitiveValue, sanitizeSensitiveText } from "../../../core/sensitive.mjs";
...
export async function enqueue(type, sessionId, payload, options = {}) {
  ...
  const safeSessionId = sanitizeSensitiveText(sessionId);
  const safePayload = sanitizeSensitiveValue(payload);
  const dedupKey = makeDedupKey(type, safeSessionId, safePayload);
  ...
  for (const { filename, entry } of pending) {
    ...
    if (entry.type === "commitSession") {
      log("pending-queue", {
        ...
        sessionId: sanitizeSensitiveText(entry.sessionId),
        ok: Boolean(res?.ok),
        status: res?.result?.status || res?.status,
        trace_id: res?.traceId || res?.result?.trace_id,
        error: res?.ok ? undefined : sanitizeSensitiveText(res?.error?.message || res?.error?.code || ""),
      });
    }
