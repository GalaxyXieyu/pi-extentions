...
export class OVClient {
    ...
    try {
      ...
      if (!resp.ok || body.status === "error") {
        return {
          ...
          result: null,
          status: resp.status,
          error: sanitizeSensitiveValue(body.error || { message: `HTTP ${resp.status}` }),
          traceId: sanitizeSensitiveText(traceId || "") || undefined,
        };
      }
      return { ok: true, result: (body.result ?? body) as T, traceId: sanitizeSensitiveText(traceId || "") || undefined };
    } catch (err: any) {
      const timedOut = err?.name === "AbortError";
      return { ok: false, result: null, status: 0, error: { code: timedOut ? "timeout" : "network_error", message: sanitizeSensitiveText(err?.message || String(err)) } };
    }
  }
  ...
  /** Public health/connection result for provider adapters. */
  ...
  async searchContext(query: string, opts?: { sessionId?: string; maxTokens?: number; purpose?: "chat" | "coding"; limit?: number }): Promise<any | null> {
    ...
    const fallback = await this.fetchJSON<any>("/api/v1/search/recall", {
      method: "POST",
      body: JSON.stringify({ query, ...(opts?.sessionId ? { session_id: opts.sessionId } : {}), limit: opts?.limit ?? 10 }),
    }, 15000);
    return fallback.ok ? fallback.result : null;
  }
