...
- [x] 支持“accept-new / keep-old / merge” + `viking_review` / `viking_memory_review` 工具。
...
- Viking Memory 生命周期入口：`providers/viking-memory/index.ts`
- Viking Memory session 捕获：`providers/viking-memory/capture.ts`
- Viking Memory provider capture/recall：`providers/viking-memory/provider.ts`
- Viking Memory API：`providers/viking-memory/client.ts`
- Viking Memory 工具注册（含 review）：`providers/viking-memory/tools.ts`
